...
const buildScope = async (userId: string) => {
...
export const taskService = {
  ...
  async listTasks(userId: string, query: Record<string, string | undefined>) {
  ...
  async updateTask(taskId: string, updates: Record<string, unknown>, userId: string, role: "admin" | "member") {
    ...
    if (role === "member") {
      ...
      task.status = String(updates.status ?? task.status) as "Todo" | "In Progress" | "Completed";
      await task.save();
      return task;
    }
